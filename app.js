...
const CANNED = [
  // General / help
  { triggers: ["help", "commands", "menu"], reply:
  ...
  // Market opportunity
  { triggers: ["business opportunity", "why herbal", "why ayurvedic", "opportunity in india"], reply:
  ...
  // Product selection
  { triggers: ["start with", "product line", "which product to start"], reply:
  ...
  // Legal & regulatory
  { triggers: ["legal requirements", "regulatory", "ayush", "fssai", "gst", "cosmetic rules"], reply:
  ...
  { triggers: ["manufacturing options", "contract manufacturing", "in-house"], reply:
  ...
  // Sourcing
  { triggers: ["sourcing", "raw materials", "where source", "suppliers"], reply:
  ...
  // Formulations
  { triggers: ["formulations", "recipes", "sample formulations", "soap formula", "hair oil formula", "serum formula"], reply:
  ...
  // Testing / QC
  { triggers: ["testing", "quality control", "qc", "stability", "heavy metal", "microbiology"], reply:
  ...
  // Labeling
  { triggers: ["labeling", "label requirements", "labelling", "label copy"], reply:
  ...
  // Packaging
  { triggers: ["packaging", "recommended packaging", "packaging recommendation"], reply:
  ...
  // Pricing
  { triggers: ["pricing", "price", "mrp ranges", "price ranges"], reply:
  ...
  // Channels
  { triggers: ["sales channels", "channels", "where sell"], reply:
  ...
  // 90-day GTM
  { triggers: ["90 days", "first 90 days", "marketing 90 days"], reply:
    "<b>90-day GTM</b><br/>" +
  ...
  // 12-month growth
  { triggers: ["12 month", "year plan", "12 months marketing"], reply:
  ...
  // Team
  { triggers: ["team", "staff", "team needed"], reply:
  ...
  // Startup cost
  { triggers: ["startup costs", "initial cost", "capex", "costs"], reply:
  ...
  // Risks
  { triggers: ["risks", "key risks", "avoid risks"], reply:
  ...
  { triggers: ["scale", "scaling", "international", "export"], reply:
  ...
  // 30-day checklist
  { triggers: ["30 days", "first 30 days", "30-day"], reply:
  ...
  // Label template
  { triggers: ["label template", "label copy", "product label"], reply:
  ...
  // Manufacturer email
  { triggers: ["manufacturer email", "sample email", "contract manufacturer email"], reply:
  ...
  { triggers: ["financial model", "12 month model", "12-month model", "financials"], reply:
  ...
  // Product list
  { triggers: ["products in plan", "what products included"], reply:
    "<b>Included products</b><br/>Herbal Hair Oil (100ml), Face Serum (30ml), Herbal Soap (100g)." },

  // MRPs & COGS
  { triggers: ["mrp list", "selling prices", "pricing list"], reply:
  ...
  { triggers: ["monthly sales", "sales volume", "assumptions"], reply:
  ...
  { triggers: ["onepager", "one page", "one-page", "one page summary"], reply:
  ...
  { triggers: ["export finance", "export finances", "download finance", "export finance csv", "export finance csv"], reply:
  ...
  { triggers: ["create pitch", "investor pitch", "one pager", "create investor pitch"], reply:
    "Create investor pitch: Type 'create pitch' to generate a 1-page pitch text for download (editable)." },

  // Fallback catch-all
  { triggers: ["default"], reply:
    "I didn't catch that. Try: 'menu', 'I know [skill]', 'start with', 'formulations', 'sourcing', 'export finance', 'onepager create', or 'create pitch'." }
];

/* --------------------------- CONTEXT & STATE --------------------------- */
const ctx = {
  selectedIdea: null,     // e.g., "Short-form Video Agency" or user's selected SKU/idea
...
function safeHTML(text) {
  // basic safe conversion for text input -> HTML (keeps allowed tags that we provide in canned replies)
  return String(text);
}
...
function findCanned(text) {
  ...
  // Exact numeric selection (if user types "1" or "2")
  if (/^\d+$/.test(t)) {
    // demo: map 1..3 to product ideas for example
    const n = parseInt(t, 10);
    if (n === 1) {
      ctx.selectedIdea = "Herbal Cold-Process Soap";
      return "<b>Selected: Herbal Cold-Process Soap</b><br/>Type 'formulations' or 'label template' or 'pricing' to get more details for this SKU.";
    }
    if (n === 2) {
      ctx.selectedIdea = "Ayurvedic Hair Oil";
      return "<b>Selected: Ayurvedic Hair Oil</b><br/>Type 'formulations' or 'label template' or 'pricing' to get more details for this SKU.";
    }
    if (n === 3) {
      ctx.selectedIdea = "Face Serum";
      return "<b>Selected: Face Serum</b><br/>Type 'formulations' or 'label template' or 'pricing' to get more details for this SKU.";
    }
  }
...
function handleSpecialCommands(userTextLower) {
  // onepager create demo / onepager create
  if (userTextLower.startsWith('onepager create')) {
    // allow 'onepager create demo' or 'onepager create' (prompt simulated)
  ...
  if (userTextLower === 'export finance' || userTextLower === 'export finance csv' || userTextLower === 'download finance' ) {
    // small CSV summarizing monthly revenue assumptions (from your earlier model approx)
    const rows = [
...
function handleSend() {
  ...
  // hide hero (first message)
  if (!heroHidden) {
    hideHeroSmooth();
  }

  // process special commands first (exact start matches)
  ...
  showTyping(650, () => {
    if (canned) {
      ...
      // If not matched, provide helpful fallback with suggestions
